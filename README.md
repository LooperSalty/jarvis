...
- **Operator (IA opérateur)** : assistant opérationnel avec onglet dédié dans le dashboard. **Trie ta boîte mail** automatiquement (classe / étiquette / archive) et te fait un **rapport** de ce qu'il a fait ; **gère tes RDV et ton agenda** (*"prends rdv mardi 14h"*) ; **écoute une réunion** (*"écoute la réunion"* / *"arrête d'écouter"*, ou import d'un fichier audio) puis en tire un compte-rendu ; **génère des devis PDF** (modèle polyvalent prestations/matériaux/produits, TVA multi-taux) **envoyés par email après ta validation « oui / non »** ; **fait des recherches internet** synthétisées. **Aucun email ni devis n'est envoyé sans ton accord explicite.** Configure ta société (pour les devis) et ton niveau d'autonomie dans *Operator → Réglages*. Nécessite l'OAuth Google (Gmail/Calendar) — un re-consentement est demandé au premier lancement (scope `gmail.modify`). PDF via `fpdf2` (optionnel), transcription via faster-whisper (optionnel).
