...
Page Vite séparée `frontend/dashboard.html` (sources `frontend/src/dashboard/`), accessible sur `http://localhost:5173/dashboard.html`, via le lien ⚙ de l'orbe, ou les entrées "Configuration" des menus tray. **Navigation à deux niveaux** (`MAIN_SECTION_IDS` dans `sections.ts`, routage dans `dashboard/main.ts`) : 4 onglets PRINCIPAUX dans la sidebar — **Chat / Cowork / Code / Automatisation** — puis un bouton **Paramètres** qui ouvre une page avec sous-onglets pour le reste : Vue d'ensemble (clés API présentes/absentes — jamais les valeurs —, intégrations, nom utilisateur), Profil (famille/adresse/habitudes/routines → injecté dans le system prompt par `jarvis_profile.contexte_profil()`), Mémoire (graphe d3-force via `graph.ts` + CRUD + **synchronisation externe**), Connecteurs (serveurs MCP + skills Jarvis), **Skills** (skills Claude Code installables en un clic), **Config Code** (Admin UI free-claude-code en iframe), Modèle IA (specs PC + reco Ollama), Personnalisation (thème + apparence de l'orbe). `DEFAULT_SECTION_ID = "chat"`.

- **Onglet Code = chat code local** (`frontend/src/dashboard/sections_code.ts` + `jarvis_actions/code_chat.py`) : onglet PRINCIPAL = un **chat spécialisé programmation** branché sur un modèle **local** via Ollama (DeepSeek Coder / Qwen, gratuit, privé — PAS le Claude payant). `code_chat.repondre()` POST `Ollama /api/chat` ; `_choisir_modele()` (pur, testé) prend le meilleur modèle de code installé (préf. `deepseek-coder` > `qwen2.5-coder` > … ; override `JARVIS_CODE_MODEL`). Handlers `dash_code_model` (modèle actif, affiché dans l'en-tête) / `dash_code_chat {prompt, history}` → `dash_code_reply {ok, text}`.
- **Config Code** (sous-onglet **Config Code** de Paramètres, `sections_code_config.ts` + `jarvis_actions/free_code.py`) : embarque l'**Admin UI de free-claude-code** (proxy `fcc-server`, providers/modèles) en **iframe** (`http://127.0.0.1:8082/admin` — aucun en-tête anti-iframe). Configure le modèle utilisé par la **commande terminal `jarvis`** (cf. mémoire `jarvis_commande_free_claude_code`). Handlers `dash_fcc_status` / `dash_fcc_start` (démarre `fcc-server` détaché) ; bouton « Démarrer le proxy » si éteint.
...
- **Ne JAMAIS versionner un `.exe` onedir** : l'exe et son `_internal/` sont **un seul artefact indivisible**. `_internal/` est gitignoré ; committer `Jarvis.exe` (ce qui avait été fait en force le 29/06, commit `7119a5f`) garantit qu'un `git pull` met à jour l'exe **seul** → binaire récent + `_internal/` périmé → `ModuleNotFoundError` **silencieux** (exe `--noconsole`) : la fenêtre s'ouvre, le backend ne démarre jamais, le WS 8765 reste fermé et Jarvis ne répond à rien. Diagnostic : `Get-NetTCPConnection -LocalPort 8765`, et comparer les dates de `Jarvis.exe` et `_internal/`. Correctif : rebuild complet (`python -m PyInstaller Jarvis.spec --clean --noconfirm`) puis déployer **exe + `_internal/` ensemble**. Les `.exe` se distribuent via les releases GitHub / l'installateur, jamais via git.
